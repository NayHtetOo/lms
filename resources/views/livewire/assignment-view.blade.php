<div class="min-h-full mt-[4rem] relative">
    ...
    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        <div class="p-4 bg-slate-100 rounded-xl transform transition-all duration-300 shadow-lg w-full flex">
            ...
            <div class="w-2/3 p-5">
                ...
                <div class="my-3 border-b border-slate-500 pb-5">
                ...
                @if ($this->isStudent)
                    ...
                    @if (isset($this->studentAssignment()->assignment_file_path) && isset($this->studentAssignment()->assignment_file_path))
                        <div class="@if ($this->studentAssignment()->received_mark)
                            border-b border-slate-700 pb-5
                        @endif" >
                            <div class="mt-3">
                    ...
                    @endif
                    @if ($this->studentAssignment() && $this->studentAssignment()->status == 2)
                        <div class="my-5">
                            <table class="w-full border border-collapse border-gray-300">
                                <thead>
                                    <tr class="300">
                                        ...
                                        <th class="px-3 py-2 text-center text-slate-800 border border-gray-300">Assignment title
                                        </th>
                                        <th class="px-3 py-2 text-center text-slate-800 border border-gray-300">Received Mark
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td class="p-3 text-center text-slate-800 border border-gray-300">
                                            {{ $this->studentAssignment()->user->name }}</td>
                                        <td class="p-3 text-center text-slate-800 border border-gray-300">
                                            {{ $this->studentAssignment()->user->email }}</td>
                                        <td class="p-3 text-center text-slate-800 border border-gray-300">
                                            {{ $this->studentAssignment()->assignment_title }}</td>
                                        <td class="p-3 text-center text-slate-800 border border-gray-300">
                                            {{ $this->studentAssignment()->received_mark }} marks
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    @endif
                    {{-- Assignment attachment file upload by student --}}
                    @if (!$this->studentAssignment() && isset($this->studentAssignment()->status) != 1)
                        <div class="my-5">
                            ...
                            <form wire:submit.prevent='saveDraft' enctype="multipart/form-data">
                                ...
                                <input class=" w-full hidden p-3 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 "
                                       id="file_input" type="file" accept="application/pdf"
                                       wire:model.live='assignmentFile' />
                        ...
                        <table class="bg-gray-200 rounded-lg w-full">
                            ...
                            <tbody>
                                @foreach ($this->allAssignments() as $assignment)
                                    <tr>
                                        <td class="py-3 px-4 text-center text-slate-700 text-sm">{{ $loop->iteration }}
                                        </td>
                                        <td class="py-3 px-4 text-center text-slate-700 text-sm">
                                            {{ $assignment->user->name }}</td>
                                        <td class="py-3 px-4 text-slate-700 text-sm flex justify-center items-center">
                                            <svg class="w-5 h-5 inline mr-2" xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <td class="py-3 px-4 text-center text-sm">
                                            @if ($assignment->status == 1)
                                                <span
                                                      class="text-md text-white inline-block bg-yellow-400 px-2 py-1 rounded-md">
                                                    in process
                                                </span>
                                            @elseif ($assignment->status == 2)
                                                <span
                                                      class="text-md text-white inline-block bg-green-400 px-2 py-1 rounded-md">
                                                    marked
                                                </span>
                                            @endif
                                        </td>
                                        <td class="py-3 px-4 text-slate-700 text-sm  text-center">
                                            <button class="border border-gray-400 p-2 rounded-md mx-1 hover:border-blue-600 hover:text-blue-600"
                                                    type="button" wire:click='markTeacher({{ $assignment->id }})'>
                ...
                @endif
                <div class="flex justify-end w-full mt-5 pb-3">
    ...
    @include('modal.assignment-teacher-marked-modal')
